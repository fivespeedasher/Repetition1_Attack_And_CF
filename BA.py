# -*- coding: gb2312 -*-
# Building a manageable BA network
...
def BA_SFN(N, M0, m) :
    ...
    # the main diagonal=0; it is a complete network
    for i in range(0, M0):
    ...
    # add 'c' node
    for c in range(M0,N):
        Allk = np.sum(AM)  # all  degree   
        ki = np.sum(AM , axis = 1)   # ki each degree for node i
        
        pi = np.zeros(c,dtype=np.float64) # probability
        for i in range(0,c):
            pi[i] = ki[i]/(Allk*1.0)
        # print pi
...
if __name__ == '__main__':
    ...
    # output Adjacency to a txt document
    ...
    # convert to Graph
    G = nx.to_networkx_graph(re_AM)
    # print(re_AM)
    pos = nx.spiral_layout(G) # 定义节点布局
    nx.draw(G,pos,node_color=range(re_AM.shape[1],0,-1),with_labels=True,cmap=plt.cm.Blues)
    plt.show()
